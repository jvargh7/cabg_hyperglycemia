...
cgmanalysis_sh <- function (df,
  ...
  files <- files[regexpr("MC",files)>0]
  ...
  dateparseorder <- c("ymd HMS","mdy HM", "mdy HMS", "mdY HM", "mdY HMS", 
                      "dmy HM", "dmy HMS", "dmY HM", "dmY HMS", "Ymd HM", "Ymd HMS", 
                      "ymd HM", "Ydm HM", "Ydm HMS", "ydm HM", "ydm HMS")
  allhours <- 0:23
  
  for (f in 1:base::length(files)) {

    table <- read_csv(paste0(inputdirectory,files[f]),skip = 0)
    

    subjectid = str_replace(files[f],"\\.(xlsx|csv)","")
    ...
    tryCatch({
      ...
      # Edited
      table <- cgm_format(table,"Pasquel") %>% 
      ...
      # Edited
      cgmupload["date_cgm_last", f] <- base::as.character(max(table$timestamp, 
                                                                   na.rm = T))
